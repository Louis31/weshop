...
Ext.onReady(function(){
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel([
	            ...
	            {header: '发布时间', width: 100, sortable:true, align:'right', dataIndex: 'date'},
        		...
        		var tbar = new Ext.Toolbar ({
        			items:[{
        	        	...
        	        	handler: function(){
        	        		var record= grid.getSelectionModel().getSelected(); 
        					if(!record){
        	                	Ext.Msg.alert('信息提示','请选择要推荐到首页的商品');
        					}else{
        		        		if(record.data.usable=="3"||record.data.usable=="2"||record.data.usable=="1"||record.data.usable=="9"){
        		        			Ext.Msg.alert('信息提示','请选择其他商品');
        		        		}else{
        		        			Ext.MessageBox.confirm('提示', '是否推荐到首页商品："'+record.data.spname+'"', function(c) {
        			 					   if(c=='yes'){
        			 					   		Ext.Ajax.request({
        			 					   			url : ctx+"/spxx/changeSpxxstate.do",
        			 					   			params:{ spid : record.data.spid,shState:6},
        			 					   			success : function(o) {
        			 					   			    var dataObj=eval("("+o.responseText+")");
        			 					   				if(dataObj.success){
        			 					   					Ext.Msg.alert("信息提示","操作成功");
        	        	...
        	        	handler: function(){
        	        		var record= grid.getSelectionModel().getSelected(); 
        					if(!record){
        	                	Ext.Msg.alert('信息提示','请选择要推荐到地区首页的商品');
        					}else{
        		        		if(record.data.usable=="3"||record.data.usable=="2"||record.data.usable=="1"||record.data.usable=="9"){
        		        			Ext.Msg.alert('信息提示','请选择其他商品');
        		        		}else{
        		        			Ext.MessageBox.confirm('提示', '是否推荐到地区首页商品："'+record.data.spname+'"', function(c) {
        			 					   if(c=='yes'){
        			 					   		Ext.Ajax.request({
        			 					   			url : ctx+"/spxx/changeSpxxstate.do",
        			 					   			params:{ spid : record.data.spid,shState:7},
        			 					   			success : function(o) {
        			 					   			    var dataObj=eval("("+o.responseText+")");
        			 					   				if(dataObj.success){
        			 					   					Ext.Msg.alert("信息提示","操作成功");
        	        	...
        	        	handler: function(){
        	        		var record= grid.getSelectionModel().getSelected(); 
        					if(!record){
        	                	Ext.Msg.alert('信息提示','请选择要取消推荐的商品');
        					}else{
        		        		if(record.data.usable=="3"||record.data.usable=="2"||record.data.usable=="1"||record.data.usable=="9"){
        		        			Ext.Msg.alert('信息提示','请选择其他商品');
        		        		}else{
        		        			Ext.MessageBox.confirm('提示', '是否取消推荐商品："'+record.data.spname+'"', function(c) {
        			 					   if(c=='yes'){
        			 					   		Ext.Ajax.request({
        			 					   			url : ctx+"/spxx/changeSpxxstate.do",
