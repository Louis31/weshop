        ...
        tbar:[{
        	...
        	handler: function(){
				...
				if(!record){
					...
					if(record.data.usable!="4"){
	        			Ext.Msg.alert('信息提示','该商品不可用，请选择其他商品');
	        		}else{
	        			var addForm = Ext.getCmp("addCMForm");
	        			addForm.getForm().findField("spname").setValue(record.get("spname"));
						addForm.getForm().findField("spid").setValue(record.get("spid"));
