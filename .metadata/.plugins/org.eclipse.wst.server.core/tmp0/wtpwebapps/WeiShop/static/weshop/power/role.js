    ...
    var grid = new Ext.grid.GridPanel({
        store: store,
      	columns : [new Ext.grid.RowNumberer(), {
			...
			menuDisabled : true,
            items: [{
                icon   : ctx+'/static/img/btn/edit2.gif',  // Use a URL in the icon config
                tooltip: '权限设置',
                handler: function(grid, rowIndex, colIndex) {
                    var record = store.getAt(rowIndex);
                    var roleid = record.get('roleid')
                	roleMenuWindow.show(null,function(){
						tree.loader.dataUrl = ctx+'/role/findRoleMenu.do?roleid='+ roleid
			...
			handler : function() {
				if (roleForm.getForm().isValid()) {
					roleForm.getForm().submit({
						url : ctx+'/role/saveOrUpdateRole.do',
