...
Ext.onReady(function(){
    ...
    var PositionTreeWindow = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				PositionTreeWindow.hide();
				addForm.getForm().findField("areacode").setValue(v_spid);
				addForm.getForm().findField("areaname").setValue(v_spname);
			}
		}, {
			width:60,
			text : '',
	...
	var Imagestore = new Ext.data.JsonStore({
	    url: ctx+'/spxx/findPageSpxxImage.do',
	    root: 'root',
	    totalProperty: 'total',
	    fields: SpxxImageObj,
	    listeners:{beforeload:function(a){a.baseParams={start:v_start, limit:v_limit,spid:v_spid};}}
	});
	//商品列表
	var sim = new Ext.grid.CheckboxSelectionModel(); 
    var Imagegrid = new Ext.grid.GridPanel({
        store: Imagestore,
        spid:null,
        cm: new Ext.grid.ColumnModel({
			...
			columns:[
	            ...
	            {header: '名称', width: 130, sortable:true, dataIndex: 'content'},
			    {header: '图片', width: 130, sortable:true, dataIndex: 'url',renderer:function(value){
				   return "<img src='"+ctx+"/upload"+value+"' style='width:130px;height:100px;'/>";
			    }}
	           ]
        }),
        sm : sim,//这里必须有
        ...
        '-',{
        	...
        	iconCls:'btn-edit',
        	handler: function(){
        		var record= Imagegrid.getSelectionModel().getSelected(); 
				if(!record){
                	Ext.Msg.alert('信息提示','请选择要预览的商品');
				}else{
					Ext.MessageBox.confirm('提示', '是否发布商品："'+record.data.content+'"', function(c) {
	 					   if(c=='yes'){
	 					   		Ext.Ajax.request({
	 					   			url : ctx+"/editorimage/deleteSpxxImage.do",
	 					   			params:{ id : record.data.id},
	 					   			success : function(o) {
	 					   			    var dataObj=eval("("+o.responseText+")");
	 					   				if(dataObj.success){
	 					   					Ext.Msg.alert("信息提示","操作成功");
	 					   				    Imagestore.reload({params:{start:v_start, limit:v_limit,spid:v_spid}});
	 					   				}else{
    ...
    var imgform = new Ext.FormPanel({
        ...
        buttons : [{
            ...
            handler : function() {
                ...
                imgform.form.submit({
                    waitMsg : '正在上传',
                    url : ctx+'/editorimage/uploadSpxxImageSig',
    ...
    var sltform = new Ext.FormPanel({
        ...
        buttons : [{
            ...
            type : 'submit',
            handler : function() {
                if (!sltform.form.isValid()) {return;}
                    sltform.form.submit({
                    waitMsg : '正在上传',
                    url : ctx+'/editorimage/uploadSpxxIcon',
                    success : function(form, action) {
                    	addForm.getForm().findField("icon").setValue(action.result.url);
                    },
                    failure : function(form, action) {
    ...
    var spxxImageWindow = new Ext.Window({
		title : '商品资源',
		width:400,
		height:450,
		closeAction:'hide',
        ...
        '-',{
        	...
        	handler: function(){
				...
				if(!record){
					...
					window.open(ctx+"/lonkoom/content.jsp?spid="+record.data.spid);
				}
        	}
        }
        ,'-',{
        	...
        	handler: function(){
        		var record= grid.getSelectionModel().getSelected(); 
				if(!record){
                	Ext.Msg.alert('信息提示','请选择要上传详情图片的商品');
				}else{
					Imagestore.reload({params:{start:v_start, limit:v_limit,spid:record.data.spid}});
					spxxImageWindow.show();
					v_spid = record.data.spid;
					imgform.getForm().findField("spid").setValue(record.data.spid);
				}
        	}
        },'-',{
        	text:'发布',
        	...
        	handler: function(){
        		var record= grid.getSelectionModel().getSelected(); 
				if(!record){
                	Ext.Msg.alert('信息提示','请选择要发布的商品');
				}else{
        			Ext.MessageBox.confirm('提示', '是否发布商品："'+record.data.spname+'"', function(c) {
	 					   if(c=='yes'){
	 					   		Ext.Ajax.request({
	 					   			url : ctx+"/spxx/changeSpxxstate.do",
	 					   			params:{ spid : record.data.spid,shState:2},
	 					   			success : function(o) {
	 					   			    var dataObj=eval("("+o.responseText+")");
	 					   				if(dataObj.success){
	 					   					Ext.Msg.alert("信息提示","操作成功");
